...
public:
    MyNode(rclcpp::NodeOptions options) : Node("my_node", options)
    {
        ...
        // init subscribers
        my_subscription = this->create_subscription<my_msgs::msg::MyInputMsg>(
      		"my_input_topic", 10, std::bind(&MyNode::my_callback, this, _1));
    }
            
        // init publishers
        my_publisher = this->create_publisher<my_msgs::msg::MyOutputMsg>("my_output_topic", 10);   // topic + QoS
      
        // init timer - the function will be called with the given rate
        publish_timer = this->create_wall_timer(100ms,    // rate
                                          [&](){callback_time();});
    }   
...
private:
    ...
    void my_callback(const my_msgs::msg::MyInputMsg & in_msg) const
    {
        my_msgs::msg::MyOutputMsg out_msg;
